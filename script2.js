...
/*************************************
 *  Scrolling animation - TILED not stretched
 *************************************/
function startScrolling(img) {
    ...
    function step() {
        ...
        // Extract row from source image
        rctx.drawImage(img, 0, nextRow, imgW, 1, 0, 0, imgW, 1);
        buffer.push(rowCanvas);

        // Limit buffer to screen height
        if (buffer.length > canvas.height) buffer.shift();

        // Clear display
        ctx.clearRect(0, 0, canvas.width, canvas.height);

        // Draw every stored row, TILED horizontally to fill screen
        for (let i = 0; i < buffer.length; i++) {
            const row = buffer[i];
            
            // Tile the row across the screen width
            const numTiles = Math.ceil(canvas.width / imgW);
            for (let tile = 0; tile < numTiles; tile++) {
                ctx.drawImage(row, 0, 0, imgW, 1, tile * imgW, i, imgW, 1);
            }
        }
